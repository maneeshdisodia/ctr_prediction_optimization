...
with open('temp', 'wb') as outcsv:
    for y in range(x2array.shape[0]):
        ...
        if 1 in x2array[y][8:15]:
            #x = np.in1d(x2array[y], click[1]).any()
            # x= np.where(pa.Index(pa.unique(click)).get_indexer(x2array[y]) >= 0)
            count = count + 1
            print count
            outcsv.write(
                "1 1.0 'B| b1:%s| b2:%s| b3:%s| b4:%s| b5:%s| b6:%s| b7:%s| b8:%s| \n" % (
                    x2array[y][8], x2array[y][9], x2array[y][10], x2array[y][11],
                    x2array[y][12], x2array[y][13], x2array[y][14], x2array[y][15]))
        else:
            outcsv.write(
                "-1 1.0 'B| b1:%s| b2:%s| b3:%s| b4:%s| b5:%s| b6:%s| b7:%s| b8:%s| \n" % (
                    x2array[y][8], x2array[y][9], x2array[y][10], x2array[y][11],
        ...
        if 1 in x2array[y][16:23]:
            #x = np.in1d(x2array[y], click[1]).any()
            # x= np.where(pa.Index(pa.unique(click)).get_indexer(x2array[y]) >= 0)
            count = count + 1
            print count
            outcsv.write(
                "1 1.0 'C| c1:%s| c2:%s| c3:%s| c4:%s| c5:%s| c6:%s| c7:%s| c8:%s| \n" % (
                    x2array[y][9], x2array[y][10], x2array[y][11],
                    x2array[y][12], x2array[y][13], x2array[y][14], x2array[y][15], x2array[y][16]))
        else:
            outcsv.write(
                "-1 1.0 'C| c1:%s| c2:%s| c3:%s| c4:%s| c5:%s| c6:%s| c7:%s| c8:%s| \n" % (
                    x2array[y][9], x2array[y][10], x2array[y][11],
                    x2array[y][12], x2array[y][13], x2array[y][14], x2array[y][15], x2array[y][16]))
        if 1 in x2array[y][24:31]:
            #x = np.in1d(x2array[y], click[1]).any()
            # x= np.where(pa.Index(pa.unique(click)).get_indexer(x2array[y]) >= 0)
            count = count + 1
            print count
            outcsv.write(
                "1 1.0 'D| d1:%s| d2:%s| d3:%s| d4:%s| d5:%s| d6:%s| d7:%s| d8:%s| \n" % (
...
os.system("vw xaa -l 1 --loss_function=logistic --link=logistic  -c -f vw_temp.model && vw -i vw_temp.model -t xab -p  /dev/stdout --quiet >> feed_prediction && awk '!seen[$0]++' feed_prediction >> ad_unit_prediction && echo Verified_and_executed|| echo commmand Down")
...
print 'sorted_results are prepared for multiclass'
